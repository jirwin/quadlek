...
import (
	...
	"github.com/jirwin/quadlek/pkg/pb/quadlek/plugins/github/v1"
...
func authFlow(cmdMsg *plugin_manager.CommandMsg, bkt *bolt.Bucket) error {
...
func issueCommand(ctx context.Context, cmdChannel <-chan *plugin_manager.CommandMsg) {
	for {
		select {
		case cmdMsg := <-cmdChannel:
			...
			if len(msg) != 2 {
				_ = cmdMsg.Helper.RespondToSlashCommand(cmdMsg.Command.ResponseUrl, &plugin_manager.CommandResp{
...
func githubAuthorizeWebhook(ctx context.Context, whChannel <-chan *plugin_manager.WebhookMsg) {
	var err error
	for {
		select {
		case whMsg := <-whChannel:
			...
			err = whMsg.Helper.Store().UpdateRaw(func(bkt *bolt.Bucket) error {
				authStateBytes := bkt.Get([]byte("authstate-" + state))
				authState := &v1.AuthState{}
				err = proto.Unmarshal(authStateBytes, authState)
				if err != nil {
					_ = whMsg.Helper.RespondToSlashCommand(authState.ResponseUrl, &plugin_manager.CommandResp{
				...
				if authState.ExpireTime < now {
					...
					err = whMsg.Helper.RespondToSlashCommand(authState.ResponseUrl, &plugin_manager.CommandResp{
						Text: "Sorry! There was an error logging you into Github.",
					})
					if err != nil {
						return err
...
func Register(id, secret, owner string) plugin_manager.Plugin {
	clientId = id
	clientSecret = secret
	defaultOwner = owner
	...
	return plugin_manager.MakePlugin(
		"github",
		[]plugin_manager.Command{plugin_manager.MakeCommand("issue", issueCommand)},
		nil,
		nil,
		[]plugin_manager.Webhook{plugin_manager.MakeWebhook("githubAuthorize", githubAuthorizeWebhook)},
		nil,
	)
}
