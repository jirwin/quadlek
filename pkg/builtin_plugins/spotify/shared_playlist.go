...
func saveSongsHook(ctx context.Context, hookChannel <-chan *plugin_manager.HookMsg) {
	for {
		select {
		case hookMsg := <-hookChannel:
			...
			err := hookMsg.Helper.Store().Get("authtoken-"+sharedPlaylistUser, func(val []byte) error {
