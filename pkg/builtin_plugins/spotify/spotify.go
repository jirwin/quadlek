...
import (
	...
	"github.com/jirwin/quadlek/pkg/pb/quadlek/plugins/spotify/v1"
...
func nowPlaying(ctx context.Context, cmdChannel <-chan *plugin_manager.CommandMsg) {
	for {
		select {
...
func spotifyAuthorizeWebhook(ctx context.Context, whChannel <-chan *plugin_manager.WebhookMsg) {
	for {
		select {
...
func Register() plugin_manager.Plugin {
	return plugin_manager.MakePlugin(
		...
		[]plugin_manager.Command{
			plugin_manager.MakeCommand("nowplaying", nowPlaying),
		},
		[]plugin_manager.Hook{
			plugin_manager.MakeHook(saveSongsHook),
		},
		nil,
		[]plugin_manager.Webhook{
			plugin_manager.MakeWebhook("spotifyAuthorize", spotifyAuthorizeWebhook),
		},
		nil,
	)
}
