...
import (
	...
	"github.com/jirwin/quadlek/pkg/pb/quadlek/plugins/gifs/v1"
...
func gifLoad(helper plugin_manager.PluginHelper) error {
	migrated := false
	err := helper.Store().Get("meta:v2Migration", func(b []byte) error {
		if b != nil {
			migrated = true
			return nil
		}
	...
	err = helper.Store().ForEach(func(bkt *bolt.Bucket, key string, value []byte) error {
	...
	err = helper.Store().Update("meta:v2Migration", []byte("true"))
	return nil
}
...
func gifCommand(ctx context.Context, cmdChannel <-chan *plugin_manager.CommandMsg) {
	for {
		select {
		case cmdMsg := <-cmdChannel:
			...
			if text != "" {
				var gifUrl string
				var err error
				err = cmdMsg.Helper.Store().Get(string(getAliasName(text)), func(v []byte) error {
					var alias *v1.Alias
					if v != nil {
						alias = &v1.Alias{}
				...
				if gifUrl != "" {
					err = cmdMsg.Helper.Store().Update(fmt.Sprintf("url:%s", gifUrl), []byte(text))
					if err != nil {
...
func gifSaveCommand(ctx context.Context, cmdChannel <-chan *plugin_manager.CommandMsg) {
	for {
		select {
		case cmdMsg := <-cmdChannel:
			...
			err = cmdMsg.Helper.Store().UpdateRaw(func(bkt *bolt.Bucket) error {
...
func gifListCommand(ctx context.Context, cmdChannel <-chan *plugin_manager.CommandMsg) {
	for {
		select {
		case cmdMsg := <-cmdChannel:
			sb := &strings.Builder{}
			err := cmdMsg.Helper.Store().ForEach(func(_ *bolt.Bucket, key string, value []byte) error {
				if len(value) == 0 {
					return nil
				}
...
func gifReaction(ctx context.Context, reactionChannel <-chan *plugin_manager.ReactionHookMsg) {
	for {
		select {
		case rh := <-reactionChannel:
			switch rh.Reaction.Reaction {
			case GoodBotReaction:
				msg, err := rh.Helper.GetMessage(rh.Reaction.Item.Channel, rh.Reaction.Item.Timestamp)
				if err != nil {
				...
				err = rh.Helper.Store().UpdateRaw(func(bkt *bolt.Bucket) error {
				...
				if err != nil {
					rh.Helper.Say(rh.Reaction.Item.Channel, "Error saving gif")
					continue
				}
...
func Register(apiKey string) plugin_manager.Plugin {
	gifs = NewGifs(apiKey, "R")
	return plugin_manager.MakePlugin(
		"gifs",
		[]plugin_manager.Command{
			plugin_manager.MakeCommand("g", gifCommand),
			plugin_manager.MakeCommand("gsave", gifSaveCommand),
			plugin_manager.MakeCommand("glist", gifListCommand),
		},
		nil,
		[]plugin_manager.ReactionHook{
			plugin_manager.MakeReactionHook(gifReaction),
		},
		nil,
		gifLoad,
	)
