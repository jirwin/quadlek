...
func load(consumerKey, consumerSecret, accessToken, accessSecret string, filter map[string]string) func(helper plugin_manager.PluginHelper) error {

	return func(helper plugin_manager.PluginHelper) error {
		go func() {
			...
			for msg := range stream.Messages {
				...
				case *twitter.Tweet:
					if channel, ok := filter[m.User.IDStr]; ok {
						...
						chanId, err := helper.GetChannelId(channel)
						if err != nil {
							zap.L().Error("unable to find channel.", zap.Error(err))
							continue
						}
						helper.Say(chanId, twitterUrl)
					}
				}
			}
		}()
...
func Register(consumerKey, consumerSecret, accessToken, accessSecret string, filter map[string]string) plugin_manager.Plugin {
